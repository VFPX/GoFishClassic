...
If Type('_Screen._GoFishClassic.oResultsForm') = 'O' And !Isnull(_Screen._GoFishClassic.oResultsForm)
	If _Screen._GoFishClassic.oResultsForm.WindowState = 1
		_Screen._GoFishClassic.oResultsForm.WindowState = 0
	Else
		_Screen._GoFishClassic.oResultsForm.Show()
	Endif
...
*-- be tricky.
	...
	_Screen.AddProperty('_GoFishClassic', loGoFish) && Add this object onto _Screen, so it can accessed from GoFish forms
...
With loThorInfo
* Required
	.Prompt		 = 'GoFish Classic' && used when tool appears in a menu
	.Description = 'Advanced Code Search Tool' && may be lengthy, including CRs, etc
	.Author 	 = 'Matt Slay'
	.PRGName	 = 'Thor_Tool_GoFishClassic'  && a unique name for the tool; note the required prefix
...
loGoFish = _Screen._GoFishClassic
